...
\begin{document}
	...
	\begin{itemize}
		\item \textbf{Επισκόπηση και Διαγνωστικά Συστήματος (System Overview \& Diagnostics):} Εμφανίζει συγκεντρωτικές πληροφορίες για την υγεία του δικτύου (Network Status), τους ενεργούς κάδους, τους αισθητήρες και τον αριθμό κρίσιμων ειδοποιήσεων. Παράλληλα, καταγράφει το σύνολο των ημερήσιων γεγονότων (Total Events Today) και την τρέχουσα εκτίμηση χωρητικότητας (Current Capacity). Η ενότητα υποστηρίζει τη δυναμική εμφάνιση κρίσιμων ειδοποιήσεων (π.χ. «CRITICAL ALERT: Bin Full») όταν η χρήση φτάσει σε οριακό επίπεδο, προτρέποντας για άμεση αποκομιδή προς αποφυγή υπερχείλισης.

		\item \textbf{Κατάσταση Κάδου (Bin Status Section):} Ένα σύνολο καρτών που αντιστοιχεί σε κάθε φυσικό κάδο του συστήματος. Παρέχει άμεσα δεδομένα όπως η τρέχουσα κατάσταση (Current Status), το επίπεδο χρήσης (Usage Level), ο συνολικός μετρητής γεγονότων (Motion Counter) και η χρονική σήμανση της τελευταίας ανίχνευσης κίνησης (Last Motion Time). Συνοδεύεται από γράφημα χρονοσειράς (Motion History) για την εποπτεία της αυξητικής τάσης της χρήσης.

		\item \textbf{Υγεία Αισθητήρων και Δικτύου (Sensor Health):} Απεικονίζει την κατάσταση συνδεσιμότητας του κόμβου (MQTT Node Connectivity), επιβεβαιώνοντας την ενεργή και απρόσκοπτη επικοινωνία (active) με τον MQTT broker.

		\item \textbf{Εικονικοί Αισθητήρες Προσομοίωσης (Mock Sensors):} Ενότητα αφιερωμένη στην παρακολούθηση μετρικών δοκιμών (testing) και αποσφαλμάτωσης, όπως το θεωρητικό ποσοστό ογκομετρικής πληρότητας (Fill Percentage) και προσομοιωμένοι μετρητές κίνησης. Το UI ενσωματώνει μηχανισμούς διαχείρισης σφαλμάτων (error handling), αναδεικνύοντας οπτικά τυχόν προβλήματα διαμόρφωσης (π.χ. «Configuration error»).

		\item \textbf{Διαγνωστικά Μηχανικής Μάθησης στο Άκρο (Edge AI Diagnostics):} Η ενότητα αυτή αφορά τον ML αισθητήρα του συστήματος. Παρουσιάζει την τρέχουσα πρόβλεψη της χρήσης του κάδου (Current Prediction, π.χ. «quiet») και το ποσοστό βεβαιότητας του μοντέλου (Model Confidence). Μέσω των γραφημάτων τηλεμετρίας (Model Telemetry) και επικάλυψης έντασης (Activity Intensity Overlay), οπτικοποιούνται οι τάσεις βεβαιότητας και καταγράφονται οι πραγματικές χρήσεις (events per window) για τις τελευταίες 12 και 6 ώρες αντίστοιχα.

		\item \textbf{Έλεγχος Στοχευμένων Ενεργειών και Ειδοποιήσεων (Targeted Action Controls \& Alerts):} Παρέχει τη διαδραστική διεπαφή αλληλεπίδρασης μεταξύ του διαχειριστή και του συστήματος. Εμφανίζει τις απαιτούμενες ενέργειες (Action Required) και επιτρέπει την παρακολούθηση ενεργών συναγερμών βάσει χρονοσφραγίδας (Alert Timestamp). Επιπλέον, μέσω των πλήκτρων ελέγχου (όπως Acknowledge Alert, Ack Target, Solve Target), ο χειριστής μπορεί να επιβεβαιώσει ή να επιλύσει τα συμβάντα, διασφαλίζοντας την ιχνηλασιμότητα των εργασιών συντήρησης.
	\end{itemize}
