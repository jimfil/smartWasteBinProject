...
def handle_sigint(sig, frame):
    global stop_flag
    print("\n[HA Virtual Sensor] Ctrl+C detected, terminating...", flush=True)
    stop_flag = True

def publish_discovery(client, bin_id, verbose):
    print(f"[HA Virtual Sensor] Publishing Home Assistant MQTT discovery configs for bin '{bin_id}'...", flush=True)
    ...
    for key, config in configs.items():
        ...
        if verbose:
            print(f"[HA Virtual Sensor] Registered entity: {config['name']} on topic: {discovery_topic}", flush=True)

def on_connect(client, userdata, flags, rc, properties=None):
    if rc == 0:
        print("[HA Virtual Sensor] Connected to MQTT broker successfully!", flush=True)
        ...
        for topic in topics:
            ...
            print(f"[HA Virtual Sensor] Subscribed to topic: {topic}", flush=True)
    else:
        print(f"[HA Virtual Sensor] Connection failed with code {rc}", flush=True)
...
def on_message(client, userdata, msg):
    ...
    if verbose:
        print(f"[HA Virtual Sensor] Received event on {topic}", flush=True)
    ...
    try:
        ...
        print(f"[HA Virtual Sensor] Error parsing telemetry payload on topic {topic}: {e}", file=sys.stderr, flush=True)
...
def main(bin_id, broker, port, verbose):
    ...
    print(f"[HA Virtual Sensor] Connecting to broker {broker}:{port}...", flush=True)

    try:
        client.connect(broker, port, keepalive=60)
    except Exception as e:
        print(f"[HA Virtual Sensor] Error connecting to broker: {e}. Exiting.", file=sys.stderr, flush=True)
        sys.exit(1)
    ...
    print("[HA Virtual Sensor] Bridge is active. Listening for mock observation events. Press Ctrl+C to stop.", flush=True)
    ...
    print("[HA Virtual Sensor] Disconnecting from broker...", flush=True)
    client.loop_stop()
    client.disconnect()
    print("[HA Virtual Sensor] Terminated successfully.", flush=True)
